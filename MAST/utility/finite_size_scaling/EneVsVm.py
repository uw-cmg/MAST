...
if __name__ == "__main__":
    ...
    for DEF in defects:
        ...
        fp.write( "          size  V_M  Eform_orig  FS_corr  Eform_corr\n")
        fp.write("======================================================\n")
        #Eform = []
        slpintcpt = dict()
        for CON in conditions:
            ...
            for i in range(len(V_M)):
                ...
                fp.write("%s  %s  %.2f  %.5f  %.5f  %.5f\n"%(blank,sizes[i],V_M[i],eform[i],FS_corr[i],Eform_corr[i]) )
                if i==len(V_M)-1: fp.write("------------------------------------------------------\n")
        fp.write( "         %s DFE    slope  corrcoeff stdev\n"%string)
        fp.write("======================================================\n")
        for con in conditions:
            mu = ""
            for ele in elements.keys():
                mu+='%.2f  '%chempot[con][ele]
            fp.write("%s  %s%.3f  %.3f    %.3f  %.3f\n"%(con,mu,slpintcpt[con][1],slpintcpt[con][0],slpintcpt[con][2],slpintcpt[con][3]) )
            fp.write("------------------------------------------------------\n")
        fp.write("\n")
    fp.close()
