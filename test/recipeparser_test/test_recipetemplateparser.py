...
class TestRecipetemplateparser(unittest.TestCase):
    ...
    def test_parse(self):
        ...
        iopt.options['neb']['neblines']=dict()
        iopt.options['neb']['neblines']['group1-group2']=dict()
        iopt.options['neb']['neblines']['group1-group3']=dict()
        ...
        myrtp=RecipeTemplateParser(inputOptions=iopt,working_directory=wd,templateFile=rt,personalRecipe=pr)
        rname=myrtp.parse()
        self.assertEquals(len(myrtp.chunks),3)
        mypr = MASTFile(pr)
